...
const THEMES = {
  dark: {
    bg: '#060611', surface: 'rgba(255,255,255,0.04)', border: 'rgba(255,255,255,0.08)',
    text: '#fff', muted: '#7ecfff', accent: '#00e5ff', accentGrad: 'linear-gradient(135deg,#00e5ff,#0072ff)',
    sidebar: '#0a0a1a', sidebarBorder: 'rgba(255,255,255,0.06)', input: 'rgba(255,255,255,0.06)',
    ...
    calBg: '#0e0e22', calHover: 'rgba(0,229,255,0.08)', calSelected: '#00e5ff',
...
function Calendar({ value, onChange, onClose, theme }) {
  ...
  return (
    <div style={{
      ...
      padding: 16, boxShadow: '0 20px 60px rgba(0,0,0,0.3)', minWidth: 280,
    }}>
      {/* Header */}
...
function DateInput({ label, value, onChange, theme }) {
  ...
  return (
    <div ref={ref} style={{ position: 'relative', flex: '1 1 140px' }}>
      ...
      {open && <Calendar value={value} onChange={onChange} onClose={() => setOpen(false)} theme={theme} />}
    </div>
  )
}
          ...
          <div>
            ...
            <div style={{ background: t.surface, border: `1px solid ${t.border}`, borderRadius: 16, padding: 24, marginBottom: 20 }}>
              ...
              <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'flex-end', marginBottom: 16, flexDirection: isMobile ? 'column' : 'row' }}>
                ...
                <DateInput label="Depart" value={depDate} onChange={setDepDate} theme={theme} />
                {isReturn && <DateInput label="Return" value={retDate} onChange={setRetDate} theme={theme} />}
